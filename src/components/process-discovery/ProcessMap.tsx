...
const ProcessMap: React.FC<ProcessMapProps> = ({ 
  ...
  const [heatmapView, setHeatmapView] = useState<'default' | 'heatmap'>('default');
  ...
  const toggleViewType = () => {
    setHeatmapView(prev => prev === 'default' ? 'heatmap' : 'default');
  };
  
  const getNodeFill = (node: Node) => {
    if (heatmapView === 'heatmap') {
  ...
  const renderProcessVisualization = () => {
    ...
    if (viewType === 'bpmn') {
      ...
      imageSrc = '/lovable-uploads/9ff06231-4f38-4cce-bea6-1cfd7a4020c2.png';
    }
        ...
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button 
                size="icon" 
                variant={heatmapView === 'heatmap' ? "default" : "ghost"}
